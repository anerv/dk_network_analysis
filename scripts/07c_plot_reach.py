...
for i, n in enumerate(network_levels_step):
    ...
    for comb in itertools.combinations(all_reach_distances, 2):
        ...
        filepaths.append(
            fp_reach_diff_pct + f"{network_levels_step[i]}_{comb[0]}-{comb[1]}"
        )

# Get subsets
plot_columns = [
    c for c in plot_columns if "diff_1_5" in c or "diff_5_10" in c or "diff_10_15" in c
]
...
df = pd.read_sql(
    f"SELECT * FROM reach.reach_{reach_dist}_component_length_hex;", engine
)
...
for i, e in enumerate([np.median, np.mean, np.max, np.std]):
    ...
    ax = sns.barplot(
        ...
        palette=list(
            distance_color_dict.values()
        ),  # sns.color_palette(pdict["cat"])[: len(all_reach_distances)],
        estimator=e,
        # width=0.8
    )
    ...
    leg = plt.legend(
        ...
        loc="upper left",
        fontsize=pdict["fs_subplot"] + 2,
        title_fontsize=pdict["fs_subplot"] + 2,
        frameon=False,
    )
    leg._legend_box.align = "left"
    plt.savefig(
        fp_reach_compare_dist_bars + labels_stat[i].lower() + ".png", dpi=pdict["dpi"]
    )
    plt.show()
    plt.close()
...
for c in comparison_types:
    ...
    # Set the labels and title
    plt.xlabel(
        f"% difference in network reach: {c.split("_")[0]} vs. {c.split("_")[1]} km",
        fontdict={"size": pdict["fs_subplot"] + 2},
    )
