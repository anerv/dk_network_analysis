...
pop = pd.read_csv("../data/input/Befolkning.csv", sep=";", encoding="utf-8")
...
geoms = gpd.read_file("../data/input/afstemningsomraade.gpkg")

geoms["ValgstedId"] = (
    geoms.kommunekode.astype(str) + "0" + geoms.afstemningsomraadenummer.astype(str)
)

geoms["ValgstedId"] = geoms.ValgstedId.str[1:]

keep_cols = ["kommunekode", "geometry", "ValgstedId", "navn"]

geoms = geoms[keep_cols]

pop["ValgstedId"] = pop.ValgstedId.astype(str)
...
# %%
# which geom is missing?
geoms.loc[geoms.ValgstedId.isin(areas.ValgstedId) == False]
# %%
